...
UI 维护批次保持原有页面结构，不更换组件库、不重构计费或权限规则：

- 收起侧栏时，分组菜单可打开子菜单，并在导航、Escape 或模式切换后关闭。
- 渠道编辑等待配置读取成功后才允许提交；失败可重试，迟到响应不能覆盖新会话。
  分组限制按会话重置，插件独立展开，折叠不会丢失已有设置。
- 渠道名称搜索同时匹配成员名称和标签，分组代表记录与总数使用相同条件；
  UI 只接受最新一次搜索、分页、排序或刷新结果。
- 令牌编辑读取失败或返回不匹配 ID 时禁止提交；按编辑目标决定更新/新建，
  保留管理员专用接口、已有设置和原始响应，旧会话读写完成不会干扰新会话。
- 结束日期打开自己的日历，不修改开始日期；默认模型图标随构建打包，
  回退图标再次失败时不循环加载。

`web/tests` 使用真实 React/MUI/Formik 和虚构接口数据；新增测试工具仅用于开发。
运行 `yarn test`、`yarn lint`、`yarn build`。CI 对 PR 和手动镜像所指定的确切提交执行
前端回归、检查和构建，失败时不能继续镜像构建。详见 `web/tests/README.md`。
模型包的另一个变动只修正整数日志的格式占位符，不改变充值、余额或分组计算。

本地模拟接口测试不能代替真实后端与浏览器验收。发布前需在隔离环境检查
管理员/普通用户、令牌读写、渠道搜索、日期统计、桌面/手机及深浅主题；
现存 lint 警告和大 bundle 提示另行分类，不以无关重构掩盖本批修复。
